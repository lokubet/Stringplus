...
START_TEST(check_const) {
  ...
  memset(str2, _i, 255);
  for (int i = 0; i < _i; i++) {
    ck_assert_ptr_eq(s21_strncat(str, str2, i), str);
    ck_assert_mem_eq(str, strncat(str3, str2, i), i);
    memset(str, 0, 256);
    memset(str3, 0, 256);
  }
}
...
START_TEST(check_dyn) {
  ...
  for (int i = 0; i < _i; i++) {
    ck_assert_ptr_eq(s21_strncat(test_string, test_string2, i), test_string);
...
START_TEST(check_dyn_over) {
  ...
  memset(test_string2, 0, 257);
  memset(test_string2, 64, 12);
  for (int i = 6; i < _i; i++) {
    ck_assert_ptr_eq(
        s21_strncat(test_string,
                    "test_string2test_string2test_string2test_string2", i),
        test_string);
    strncat(test_string2, "test_string2test_string2test_string2test_string2",
            i);
    ck_assert_mem_eq(test_string, test_string2, i);
    memset(test_string, 0, 257);
    memset(test_string, 64, 12);
